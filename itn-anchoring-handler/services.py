...
def get_azure_data_tables_data(media_id: str):
    ...
    with TableServiceClient(
        ...
        return AzureData(datetime.strptime(timestamp, "%y-%m-%d %H:%M:%S"), data["AspectRatio"],
                         datetime.strptime(str(data["Created"]), "%y-%m-%d %H:%M:%S"), data["Codec"],
...
def get_xendata(media_id: str):
    ...
    return Xendata(
        creation_date=datetime.strptime(row[0], "%y-%m-%dT%H:%M:%S"),
        modification_date=datetime.strptime(row[1], "%y-%m-%dT%H:%M:%S"),
        size=int(row[2])
    )
